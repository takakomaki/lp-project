...
export function HeavenSection() {
  const copy = lpCopyVol01.heaven
  return (
      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {copy.scenes.map((s) => (
            ...
            >
              <div className="text-[13px] font-semibold text-neutral-900">{s.title}</div>
              <p className="mt-3 text-[13px] leading-relaxed text-neutral-700">{s.body}</p>
            </div>
          ))}
        </div>
      </div>
